...
import { AgentSpeechManager } from './managers/AgentSpeechManager'
...
export class TldrawAgent {
	...
	/** The accessibility manager (cognitive/visual aids). */
	accessibility: AgentAccessibilityManager
	...
	constructor({ editor, id, onError }: TldrawAgentOptions) {
		...
		this.accessibility = new AgentAccessibilityManager(this)
	...
	async preparePrompt(request: AgentRequest, helpers: AgentHelpers): Promise<AgentPrompt> {
		...
		const prompt = Object.fromEntries(transformedParts.map((part) => [part.type, part])) as AgentPrompt

		// Inject accessibility instructions into the messages part
		const a11yInstructions = this.accessibility.getPromptInstructions()
		if (a11yInstructions && prompt.messages) {
			const messagesPart = prompt.messages as { type: string; agentMessages: string[]; requestSource: string }
			messagesPart.agentMessages = [a11yInstructions, ...messagesPart.agentMessages]
		}
						...
						editor.run(
							() => {
								...
								if (transformedAction.complete) {
									...
									// Speak the action's text aloud
									this.speech.handleCompletedAction(transformedAction)
								} else {
									incompleteDiff = diff
								}
							},
