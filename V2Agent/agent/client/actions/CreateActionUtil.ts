...
export const CreateActionUtil = registerActionUtil(
	class CreateActionUtil extends AgentActionUtil<CreateAction> {
		...
		override applyAction(action: Streaming<CreateAction>, helpers: AgentHelpers) {
			...
			// Accessibility: zoom to shape + highlight pulse on complete
			if (action.complete && result.shape.id) {
				const a11y = this.agent.accessibility.getSettings()

				if (a11y.zoomToShape) {
					const bounds = editor.getShapePageBounds(result.shape.id)
					if (bounds) {
						editor.zoomToBounds(bounds.expandBy(100), { animation: { duration: 400 } })
					}
				}

				if (a11y.highlightNewShapes) {
					const el = document.querySelector(`[data-shape-id="${result.shape.id}"]`)
					if (el) {
						el.classList.add('a11y-shape-pulse')
						setTimeout(() => el.classList.remove('a11y-shape-pulse'), 1500)
					}
				}

				if (a11y.focusMode) {
					// Dim all shapes except the newly created one
					document.querySelectorAll('.tl-shape').forEach((shapeEl) => {
						if (shapeEl.getAttribute('data-shape-id') !== String(result.shape!.id)) {
							shapeEl.classList.add('a11y-dimmed')
						} else {
							shapeEl.classList.remove('a11y-dimmed')
						}
					})
					// Clear dimming after a delay
					setTimeout(() => {
						document
							.querySelectorAll('.a11y-dimmed')
							.forEach((el) => el.classList.remove('a11y-dimmed'))
					}, 3000)
				}
			}
